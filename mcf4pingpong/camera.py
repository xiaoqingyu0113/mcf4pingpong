...
class CameraParam:
    ...
    def undistort_pixel(self,uv):
        return undistort_pixel(uv,self.d,self.K)
    ...
    def test(self):
        K1_gtsam,pose1 = self.to_gtsam()
        camera1_gtsam = gtsam.PinholeCameraCal3_S2(pose1, K1_gtsam)
        N = 10
        pN = np.random.rand(N,3)/3.0
        for p0 in pN: 
            print('------------')
            print(self.distort_pixel(camera1_gtsam.project(p0)))
            print(self.proj2img(p0))
...
def triangulate(uv_left, uv_right, left_camera_param:CameraParam, right_camera_param:CameraParam):
    """
    Triangulate the 3D position of the ball given its 2D position in two stereo images.

    Returns:
        ball_position: 3D coordinates of the ball in world coordinates.
    """
    ...
    # if no undistort
    # homogeneous_2d_points_left = uv_left[None,None,:]
    # homogeneous_2d_points_right = uv_right[None,None,:]
